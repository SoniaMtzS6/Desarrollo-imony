...
public class SettlementsController {
    ...
    public ResponseEntity<SettlementsReportResponse> generateReport(
            @RequestHeader("X-Pomelo-Token") String pomeloToken,
            @RequestHeader("X-Pomelo-Base-Url") String pomeloBaseUrl,
        ...
        try {
            SettlementsReportResponse response = settlementsService.generateReport(startDate, endDate, accountId, page, size, pomeloToken, pomeloBaseUrl);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            log.error("Error en endpoint /report: {}", e.getMessage(), e);
    ...
    public ResponseEntity<SettlementsReportResponse> getSettlements(
        ...
        try {
            ...
            List<SettlementData> settlements = settlementsService.getSettlements(startDate, endDate, accountId, pomeloToken, pomeloBaseUrl);
            ...
            log.error("Error en endpoint /settlements: {}", e.getMessage(), e);
            SettlementsReportResponse errorResponse = new SettlementsReportResponse();
            errorResponse.setSuccess(false);
            errorResponse.setMessage("Error interno del servidor: " + e.getMessage());
            return ResponseEntity.internalServerError().body(errorResponse);
    ...
    public ResponseEntity<SettlementsReportResponse> getTransactions(
        ...
        try {
            ...
            SettlementsReportResponse response = new SettlementsReportResponse();
            List<TransactionData> transactions = settlementsService.getTransactions(startDate, endDate, accountId, pomeloToken, pomeloBaseUrl);
            ...
            log.error("Error en endpoint /transactions: {}", e.getMessage(), e);
            SettlementsReportResponse errorResponse = new SettlementsReportResponse();
            errorResponse.setSuccess(false);
            errorResponse.setMessage("Error interno del servidor: " + e.getMessage());
            return ResponseEntity.internalServerError().body(errorResponse);

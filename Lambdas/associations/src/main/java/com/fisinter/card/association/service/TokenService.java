...
import com.fisinter.card.association.model.TokenRequest;
import com.fisinter.card.association.model.TokenResponse;
				...
				.onStatus(HttpStatusCode::is4xxClientError,
						responseLam -> responseLam.bodyToMono(String.class).flatMap(error -> {
							...
							TokenError tokenError = switch(code) {
								case 400 -> new TokenError(code, "Bad Request", error);
								case 403 -> new TokenError(code, "Access Denied", error);
								case 401 -> new TokenError(code, "Unauthorized", error);
								default -> new TokenError(code, "Unexpected error", error);};
